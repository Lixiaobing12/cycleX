...
const DrawerGetBlindBox = () => {
    ...
    const open = () => {
        request.post("/api/lottery/addNum", {
            BearerToken: "Bearer " + accessToken?.token,
            Type: "Login"
        })
        const context: any = modal?.info({
            closable: {
                closeIcon: <Icon size={25}><CloseCircleOutlineds /></Icon>
            },
            icon: <></>,
            ...
            content: (
                <div className="bg-white rounded-box flex flex-col justify-center items-center">
                    ...
                    <button className="btn btn-wide m-auto mt-4 bg-black text-white hover:bg-black hover:scale-105" onClick={() => {
                        context.destroy();
                        navigate("/blindBox")
                    }}>
                        Open
